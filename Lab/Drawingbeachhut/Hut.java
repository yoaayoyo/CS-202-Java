...
public class Hut extends JPanel {  
	...
	public void paintComponent(Graphics g) {
		...
		g.fillRect(0, 0, 600, 600);   // the first two number for x,y starts from the top left corner.
			...
			g.fillRect(175, 160+moveitup, 100, 120);
			//roof
			g.setColor(Color.pink);
			g.fillRect(160, 130+moveitup, 135, 50);
			//door
			g.setColor(Color.red); 
			g.fillRect(215, 235+moveitup, 20, 45);
			//window
			g.setColor(Color.red);
			g.fillRect(185, 195+moveitup, 20, 20);
			//word
			g.setColor(Color.BLACK);
			g.drawRect(210, 185+moveitup, 63,12 );
			g.drawString("Sheng's hut", 210, 195+moveitup);
		...
		g.setColor(Color.WHITE);
		if (moveitup < 0) {
			g.drawString("the hut is flying")
		 } else {
			g.drawString("on the ground");
		 }
		 if (yesno.eqaul)
